...
let currentUserDataDir = null; // 记录当前临时目录
let currentOutputDir = null;   // 记录当前输出目录
// 文件流变量
let logStream = null;
// 全局状态/日志管理
let crawlerState = {
    ...
    error: null,            // 错误信息
    currentDir: '',          // 记录当前检索的目录
    logIndex: 0 // 记录前端已读取的日志索引
};
...
async function extractCompleteResult(result, searchKeyword, originalPaper, isMatch, searchType, resultCountFormatted, page) {
    ...
    paperInfo.citationLink = citationLink; // 保存引用链接
...
function recordFailedPaperData(originalPaper, searchKeyword, failureReason, resultCountFormatted) {
    ...
    failedPaper.citationLink = ''; // 添加空链接
...
function writeToExcel(dataList, filePath, sheetName) {
    ...
    dataList.forEach((paper, rowIndex) => {
        ...
        worksheet.cell(row, 5).string(paper.citationLink || "");
        worksheet.cell(row, 6).string(paper.year || "");
        worksheet.cell(row, 7).string(paper.publication || "");
        worksheet.cell(row, 8).string(paper.remark || "");
        worksheet.cell(row, 9).string(paper.searchTime || "");
        worksheet.cell(row, 10).string(paper.resultCountFormatted || "");
        worksheet.cell(row, 11).string(paper.endNoteLink || "");
        worksheet.cell(row, 12).string(paper.downloadedFilePath || "");
    });
